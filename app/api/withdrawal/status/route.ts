...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('❌ Withdrawal status action error:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
async function handleRefreshWithdrawalStatus(userId: string, transactionId: string) {
  try {
    ...
    console.error('❌ Refresh withdrawal status failed:', error);
    return NextResponse.json(
...
async function handleGetWithdrawalStatistics(userId: string) {
  try {
    ...
    if (error) {
      console.error('❌ Failed to get user withdrawal stats:', error);
    }
    ...
    console.error('❌ Get withdrawal statistics failed:', error);
    return NextResponse.json(
