...
class LuaScriptPerformanceTest {
  ...
  async testLuaScriptBasicOperation() {
    return await this.runTest('Lua Script Basic Operation', async () => {
      ...
      return {
        ...
        success: result.value === testValue,
        timestamp: result.timestamp
      };
    });
  }

  async testLuaScriptOrderMatching() {
    return await this.runTest('Lua Script Order Matching', async () => {
      console.log('Testing Lua script order matching logic...');
      
      const orderMatchingScript = \`
        local orderbook_key = KEYS[1]
        local new_order = cjson.decode(ARGV[1])
        
        -- Simple order matching simulation
        local matches = {}
        local remaining_amount = tonumber(new_order.amount)
        
        -- Mock orderbook data for testing
        local opposite_side = new_order.side == 'buy' and 'asks' or 'bids'
        local mock_orders = {
          {id = 'mock1', price = '1.00', amount = '500'},
          {id = 'mock2', price = '1.01', amount = '300'},
          {id = 'mock3', price = '1.02', amount = '200'}
        }
        
        for i, order in ipairs(mock_orders) do
          if remaining_amount <= 0 then break end
          
          local match_amount = math.min(remaining_amount, tonumber(order.amount))
          
          table.insert(matches, {
            id = 'match_' .. i,
            price = order.price,
            amount = tostring(match_amount),
            timestamp = redis.call('TIME')[1]
          })
        ...
        return cjson.encode({
          matches = matches,
          remaining = tostring(remaining_amount),
          total_matches = #matches
        })
      `;
      
      const orderbookKey = 'orderbook:HYPERINDEX-USDC';
      const testOrder = {
      ...
      const startTime = Date.now();
      const result = await this.redis.eval(orderMatchingScript, 1, orderbookKey, JSON.stringify(testOrder));
      const endTime = Date.now();
      ...
      return {
        ...
        totalMatches: matchResult.total_matches,
        remainingAmount: matchResult.remaining,
        matches: matchResult.matches,
        executionTime: endTime - startTime,
        luaPerformance: 'atomic_matching'
      };
    });
  }
  ...
  async generateReport() {
    ...
    this.testResults.tests.forEach(test => {
      ...
      if (test.status === 'failed') {
        ...
        if (test.name === 'Lua Script Order Matching' && test.data.totalMatches) {
          console.log(\`   Matches: \${test.data.totalMatches}, Remaining: \${test.data.remainingAmount}\`);
          console.log(\`   Execution: \${test.data.executionTime}ms\`);
        }
  ...
  async runAllTests() {
    ...
    try {
      ...
      await this.testLuaScriptOrderMatching();
