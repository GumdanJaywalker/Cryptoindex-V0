...
/**
 * HOOATS Standalone API Server (Fixed Version)
 * 
...
let HybridSmartRouterV2: any, ParallelMatchingEngine: any, UltraPerformanceOrderbook: any, AsyncDBWriter: any;
let extractPrivyAuthFromRequest: any;

/**
 * 개발용 인증 미들웨어 (Bearer dev-token)
 */
function devAuth(req: AuthenticatedRequest, res: express.Response, next: express.NextFunction): void {
...
/**
 * 실제 모듈들을 동적으로 로드 (절대 경로 사용)
 */
async function loadHOOATSModules(): Promise<void> {
  try {
    console.log('📦 Loading HOOATS modules...');

    // 절대 경로로 TypeScript 모듈들을 동적 import
    const smartRouterPath = path.join(__dirname, 'lib', 'trading', 'smart-router-v2.ts');
    const SmartRouterModule = await import(smartRouterPath);
    HybridSmartRouterV2 = SmartRouterModule.HybridSmartRouterV2;
    
    const parallelMatchingPath = path.join(__dirname, 'lib', 'orderbook', 'parallel-matching-engine.ts');
    const ParallelMatchingModule = await import(parallelMatchingPath);
    ParallelMatchingEngine = ParallelMatchingModule.ParallelMatchingEngine;
    
    const ultraOrderbookPath = path.join(__dirname, 'lib', 'orderbook', 'ultra-performance-orderbook.ts');
    const UltraOrderbookModule = await import(ultraOrderbookPath);
    UltraPerformanceOrderbook = UltraOrderbookModule.UltraPerformanceOrderbook;
    
    const asyncDBPath = path.join(__dirname, 'lib', 'utils', 'async-db-writer.ts');
    const AsyncDBModule = await import(asyncDBPath);
    AsyncDBWriter = AsyncDBModule.AsyncDBWriter;

    const authPath = path.join(__dirname, 'lib', 'middleware', 'privy-auth.ts');
 ...
 */
app.get('/api/redis/status', devAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const matchingEngine = ParallelMatchingEngine.getInstance();
    const metrics = matchingEngine.getMetrics();
 ...
 */
app.post('/api/trading/v2/orders', devAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const user = req.user!;
    const body = req.body;
...
app.get('/api/trading/v1/orderbook', async (req, res) => {
  try {
    ...
    const pair = url.searchParams.get('pair') || 'HYPERINDEX-USDC';
 ...
 */
app.get('/api/trading/v1/market', devAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const url = new URL(req.url!, `http://${req.headers.host}`);
    const pair = url.searchParams.get('pair') || 'HYPERINDEX-USDC';
...
async function initializeHOOATS(): Promise<void> {
