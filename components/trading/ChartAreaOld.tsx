...
// Trading Data Content Component
function TradingDataContent() {
  const [expandedChart, setExpandedChart] = useState<string | null>(null)
  const [timeframe, setTimeframe] = useState('1h')

  // Mock data for charts
  const generateMockData = (points: number, baseValue: number, volatility: number = 0.1) => {
    const data = []
  ...
  const indexVolumeData = generateMockData(24, 50000000, 10000000)
  const buyerRatioAccountsData = generateMockData(24, 1.5, 0.3)
  const buyerRatioPositionsData = generateMockData(24, 1.8, 0.4)
  const buyPressureData = generateMockData(24, 55, 10)
  const premiumData = generateMockData(24, 0.05, 0.02)

  // Chart component
  const MiniChart = ({ title, data, color, type = 'line', suffix = '', showFullscreen = true }: {
    title: string
    data: any[]
    color: string
    type?: 'line' | 'bar'
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-right">
            <div className="text-xs font-semibold text-white">
              {formatValue(data[data.length - 1]?.value || 0)}{suffix}
            </div>
            <div className={`text-xs ${data[data.length - 1]?.value > data[data.length - 2]?.value ? 'text-green-400' : 'text-red-400'}`}>
            ...
            {type === 'line' ? (
              <>
                ...
                {data.map((d, i) => {
                  ...
                  const y = 55 - ((d.value - minValue) / range) * 50
                  return (
                    <circle key={i} cx={x} cy={y} r="1" fill={color} opacity={i === data.length - 1 ? 1 : 0} />
                  )
                })}
              </>
            ) : (
              data.map((d, i) => {
                const x = (i / data.length) * 190 + 5
                const height = ((d.value - minValue) / range) * 50
                const y = 55 - height
                return (
                  <rect key={i} x={x} y={y} width={190 / data.length - 1} height={height} fill={color} opacity={0.7} />
                )
              })
            )}
          </svg>
  ...
  // Volume Distribution Chart (Buy/Sell Bars)
  const VolumeDistributionChart = () => {
    const buyVolume = generateMockData(12, 30000000, 5000000).map(d => ({ ...d, type: 'buy' }))
    ...
    return (
      <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-3 relative">
        ...
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-xs font-medium text-white">Volume Distribution</h4>
          <div className="text-right">
            <div className="text-xs text-green-400">Buy: 62.3%</div>
            <div className="text-xs text-red-400">Sell: 37.7%</div>
          </div>
        </div>
        ...
        <div className="h-16 relative">
          <svg viewBox="0 0 200 60" className="w-full h-full">
            {buyVolume.map((d, i) => {
              const x = (i / buyVolume.length) * 190 + 5
              const maxVolume = Math.max(...buyVolume.map(v => v.value), ...sellVolume.map(v => v.value))
              const buyHeight = (d.value / maxVolume) * 25
              const sellHeight = (sellVolume[i]?.value / maxVolume) * 25
              
              return (
                <g key={i}>
                  <rect x={x} y={30 - buyHeight} width={190 / buyVolume.length - 1} height={buyHeight} fill="rgb(34 197 94)" opacity={0.8} />
                  <rect x={x} y={30} width={190 / buyVolume.length - 1} height={sellHeight} fill="rgb(239 68 68)" opacity={0.8} />
                </g>
              )
            })}
          </svg>
        ...
        <div className="flex-1 p-8">
          <div className="h-full bg-slate-900/50 rounded-lg p-6">
            <div className="text-center text-slate-400 pt-20">
              ...
              <div className="text-sm">Full-featured chart implementation would go here</div>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full bg-slate-950 overflow-y-auto">
      <div className="p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 max-h-[calc(100vh-200px)] overflow-y-auto">
          {/* Index Volume */}
          <MiniChart 
            title="Index Volume" 
            data={indexVolumeData} 
            color="rgb(34 197 94)" 
            type="bar"
            suffix=""
          />
          ...
          <MiniChart 
            title="Top Trader Buy/Sell Ratio (Accounts)" 
            data={buyerRatioAccountsData} 
          ...
          <MiniChart 
            title="Top Trader Buy/Sell Ratio (Positions)" 
            data={buyerRatioPositionsData} 
