    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-slate-900/50 border-slate-800"><CardContent className="p-4">
          ...
          <div className="text-lg font-semibold">{formatUsd(summary.total)}</div>
        </CardContent></Card>
        <Card className="bg-slate-900/50 border-slate-800"><CardContent className="p-4">
          <div className="text-xs text-slate-400 mb-1">{tf==='24h'?'24H': tf==='7d'?'7D': tf==='30d'?'30D':'Pending'}</div>
          <div className="text-lg font-semibold">{tf==='all'? formatUsd(summary.pending): tf==='24h'? formatUsd(summary.d24): tf==='7d'? formatUsd(summary.d7): formatUsd(summary.d30)}</div>
        </CardContent></Card>
        <Card className="bg-slate-900/50 border-slate-800"><CardContent className="p-4">
          <div className="text-xs text-slate-400 mb-1">Paid to Date</div>
          <div className="text-lg font-semibold">{formatUsd(summary.paid)}</div>
        </CardContent></Card>
        <Card className="bg-slate-900/50 border-slate-800"><CardContent className="p-4">
          <div className="text-xs text-slate-400 mb-1">Pending Payouts</div>
          <div className="text-lg font-semibold">{formatUsd(summary.pending)}</div>
        </CardContent></Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="bg-slate-900/50 border-slate-800"><CardContent className="p-4">
          ...
          <div className="text-lg font-semibold">{formatUsd(Math.round(summary.total * 0.6))}</div>
        </CardContent></Card>
        <Card className="bg-slate-900/50 border-slate-800"><CardContent className="p-4">
          <div className="text-xs text-slate-400 mb-1">LP Fee (liquidity pool trading fees)</div>
          <div className="text-lg font-semibold">{formatUsd(Math.round(summary.total * 0.4))}</div>
        </CardContent></Card>
      </div>
        ...
        <div className="divide-y divide-slate-800">
          {mockPerIndex.map((r) => (
            <div key={r.index} className="grid grid-cols-8 px-4 py-2 items-center text-sm">
              ...
              <div className="text-right text-slate-300">{formatUsd(r.tvl)}</div>
              <div className="text-right text-slate-300">{formatUsd(r.volume24h)}</div>
              <div className="text-right text-slate-300">{(r.creatorFeeRate*100).toFixed(2)}%</div>
              <div className="text-right text-slate-300">{formatUsd(r.earnings24h)}</div>
              <div className="text-right text-slate-300">{formatUsd(r.earnings7d)}</div>
              <div className="text-right text-slate-300">{formatUsd(r.earnings30d)}</div>
              <div className="text-right text-slate-300">{formatUsd(r.earningsTotal)}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="divide-y divide-slate-800">
          {mockPayouts.map(p => (
            <div key={p.id} className="grid grid-cols-5 px-4 py-2 items-center text-sm">
              ...
              <div className="text-slate-300">{formatUsd(p.amount)}</div>
