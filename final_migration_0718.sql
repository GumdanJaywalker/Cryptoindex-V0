-- Final Migration for P2PFiat Platform
-- Date: 2025-07-18
-- Description: Complete database schema with all security, fraud detection, and user management features
...
CREATE TABLE IF NOT EXISTS users (
  ...
  auth_type TEXT CHECK (auth_type IN ('email', 'wallet')),
  email TEXT,
  wallet_address TEXT,
  ...
  -- Constraints
  CONSTRAINT users_auth_consistency CHECK (
    (auth_type = 'email' AND email IS NOT NULL) OR 
    (auth_type = 'wallet' AND wallet_address IS NOT NULL)
  )
);

-- User wallets table (for email users with embedded wallets)
CREATE TABLE IF NOT EXISTS user_wallets (
  ...
  wallet_type TEXT DEFAULT 'embedded' CHECK (wallet_type IN ('embedded', 'external')),
  is_primary BOOLEAN DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  
  -- Constraints
  CONSTRAINT user_wallets_user_unique UNIQUE(user_id, embedded_wallet_address)
);
...
-- ============================================================================
-- NETWORK AND HYPERLIQUID SPECIFIC TABLES
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_user_wallets_embedded_wallet_address ON user_wallets(embedded_wallet_address);
...
-- Note: Detailed RLS policies should be created based on auth.uid() 
-- This requires Supabase's auth system to be properly configured
...
INSERT INTO security_audit_log (
    ...
    'success',
    jsonb_build_object(
        'migration_file', 'final_migration_0718.sql',
        'migration_date', NOW(),
        'tables_created', ARRAY[
            'users', 'user_wallets', 'transactions', 'user_behavior_patterns',
            'fraud_analysis_logs', 'security_incidents', 'security_audit_log',
            'user_devices', 'ip_address_tracking', 'rate_limiting', 'network_configs'
        ]
    )
);

-- Migration completed successfully!
SELECT 'P2PFiat Database Migration Completed Successfully!' as status;