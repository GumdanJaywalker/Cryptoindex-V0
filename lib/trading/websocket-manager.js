...
export class WebSocketManager {
    static instance;
    connections = new Map();
    subscriptions = new Map(); // type -> connectionIds
     ...
     */
    addConnection(connectionId, userId, socket) {
        const connection = {
            id: connectionId,
            userId,
            socket,
     ...
     */
    unsubscribe(connectionId, types) {
        const connection = this.connections.get(connectionId);
        if (!connection) {
            return { success: false, error: 'Connection not found' };
        }
        // Remove from connection subscriptions
        connection.subscriptions = connection.subscriptions.filter(sub => !types.some(type => sub.type.includes(type)));
     ...
     */
    broadcastPriceUpdate(tokenAddress, price, change24h) {
        const message = {
            type: 'price_update',
            data: {
                tokenAddress,
     ...
     */
    broadcastBalanceUpdate(userId, tokenAddress, balanceData) {
        const message = {
            type: 'balance_update',
            data: {
                tokenAddress,
     ...
     */
    broadcastTradeExecuted(tokenAddress, tradeData) {
        const message = {
            type: 'trade_executed',
            data: {
                trade: tradeData,
