...
export class AdvancedOrderValidator {
     ...
     * 종합 주문 검증
     */
    static async validateOrder(order, userBalances) {
     ...
     * 기본 형식 검증
     */
    static validateFormat(price, amount, type) {
        // 수량 형식 검증
     ...
     * 가격 검증 및 틱 스냅
     */
    static validatePrice(price, config) {
        try {
            // 틱 사이즈 검증
     ...
     * 수량 및 거래 금액 검증
     */
    static validateAmount(price, amount, config) {
        try {
            // 정밀도 기반 검증
     ...
     * 잔액 검증
     */
    static async validateBalance(order, balances, config) {
     ...
     * 시장 상황 검증
     */
    static validateMarketConditions(price, config) {
        const warnings = [];
        // 여기서는 간단한 예시만 구현
     ...
     */
    static async estimateMarketPrice(pair, side) {
        try {
            ...
            try {
                ...
                try {
                    ...
                    // Pair 계약에서 직접 리저브 조회
                    const pairContract = new ethers.Contract('0x5706084ad9Cac84393eaA1Eb265Db9b22bA63cd1', [
            ...
            console.error('❌ Market price estimation completely failed:', error);
            return '1.0'; // 안전한 기본값
        }
    }
    /**
     * 빠른 검증 (UI용)
     */
    static quickValidate(price, amount, type, pair) {
     ...
     * 🧪 테스트용: 보안 공격 시나리오 검증
     */
    static validateSecurityScenarios(price, amount, type) {
     ...
     */
    static validateEdgeCases(order) {
     ...
     * 🧪 테스트용: 성능 스트레스 테스트 검증
     */
    static validatePerformanceStress(orders) {
        ...
        for (const order of orders) {
            const quickResult = this.quickValidate(order.price, order.amount || '0', order.type || 'market', order.pair || 'HYPERINDEX-USDC');
     ...
     */
    static setTestMode(enabled, testnetConfig) {
        // 실제 테스트넷 모드 설정
        if (enabled) {
            // 테스트넷 환경 변수 설정
            ...
            if (testnetConfig?.mockPricesOnFailure) {
                global.__TESTNET_FALLBACK_PRICES__ = testnetConfig.mockPricesOnFailure;
            }
            else {
                global.__TESTNET_FALLBACK_PRICES__ = {
                    'HYPERINDEX-USDC': 1.0,
                    'PEPE-USDC': 0.0000012,
                    'DOGE-USDC': 0.08,
                    'SHIB-USDC': 0.000024
            ...
            delete global.__TESTNET_FALLBACK_PRICES__;
     ...
     */
    static async generateValidationReport(order, userBalances) {
        const startTime = Date.now();
        ...
        // 보안 검증
        const securityCheck = this.validateSecurityScenarios(order.price, order.amount || '0', order.type || 'market');
        // Edge Case 검증
     ...
     * 🚀 실제 테스트넷 통합 검증
     */
    static async validateTestnetIntegration() {
        try {
            const { ethers } = await import('ethers');
            const provider = new ethers.JsonRpcProvider('https://api.hyperliquid-testnet.xyz/evm');
            // 네트워크 연결 확인
            ...
            let gasEstimate;
            try {
                const routerContract = new ethers.Contract('0xD70399962f491c4d38f4ACf7E6a9345B0B9a3A7A', ['function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts)'], provider);
                const amounts = await routerContract.getAmountsOut(ethers.parseEther('1'), ['0x6065Ab1ec8334ab6099aF27aF145411902EAef40', '0x53aE8e677f34BC709148085381Ce2D4b6ceA1Fc3']);
            ...
            console.error('Testnet integration validation failed:', error);
            return {
