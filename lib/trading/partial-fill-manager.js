...
export class PartialFillManager {
     ...
     */
    async executePartialFill(order, fillAmount, executionPrice, counterpartyOrderId, isMarketMaker = false) {
        try {
            ...
            const validation = this.validateFillAmount(fillAmount, order.remainingAmount, tokenDecimals);
            ...
            const newRemainingAmount = PrecisionUtils.calculateRemainingAmount(order.remainingAmount, fillAmount, tokenDecimals);
            // Calculate new total filled amount
            const newFilledAmount = PrecisionUtils.safeAdd(order.filledAmount, fillAmount, tokenDecimals);
            // Calculate fill percentage
            const fillPercentage = PrecisionUtils.calculateFillPercentage(fillAmount, order.amount, tokenDecimals);
            // Calculate fees
            const fees = this.calculateFillFees(fillAmount, executionPrice, isMarketMaker, tokenDecimals);
            // Generate fill ID
            ...
            const newAveragePrice = PrecisionUtils.calculateAverageFillPrice(allFills.map(fill => ({
                amount: fill.fillAmount,
                price: fill.executionPrice
            })), tokenDecimals);
            // Calculate total fees
            const totalFees = allFills.reduce((total, fill) => PrecisionUtils.safeAdd(total, fill.fees.total, tokenDecimals), '0');
            // Determine order status
            const newStatus = PrecisionUtils.determineOrderStatus(newFilledAmount, order.amount, tokenDecimals);
            // Map precision utils status to order status
            const mappedStatus = newStatus === 'filled' ? 'filled' :
     ...
     */
    async getOrderFillSummary(orderId) {
        try {
            ...
            // Calculate fill percentage
            const fillPercentage = PrecisionUtils.calculateFillPercentage(orderData.filled_amount, orderData.amount, 18 // Default decimals
            );
            // Get last fill timestamp
            const lastFill = fills.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())[0];
            return {
                orderId,
            ...
            console.error('❌ Failed to get order fill summary:', error);
            return null;
        }
    }
    /**
     ...
     */
    validateFillAmount(fillAmount, remainingAmount, decimals) {
        try {
            // Validate format
     ...
     */
    calculateFillFees(fillAmount, executionPrice, isMarketMaker, decimals) {
        try {
            ...
            const feeAmount = PrecisionUtils.safeMultiply(fillValue, feeRate.toString(), decimals);
            return {
     ...
     */
    async fixPrecisionIssues(orderId) {
        try {
            ...
            const calculatedRemaining = PrecisionUtils.calculateRemainingAmount(orderData.amount, orderData.filled_amount, tokenDecimals);
            const isEqual = PrecisionUtils.isAmountEqual(calculatedRemaining, orderData.remaining_amount, tokenDecimals);
            if (!isEqual) {
            ...
            console.error('❌ Failed to fix precision issues:', error);
            return {
                success: false,
                fixed: false,
