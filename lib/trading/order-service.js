...
export class TradingOrderService {
    ...
    async createOrder(request) {
        try {
            ...
            if (request.side === 'buy') {
                const hasBalance = await this.validateBuyOrderBalance(request.userId, request.tokenAddress, request.amount, request.price, request.type);
                if (!hasBalance.valid) {
                    return {
                        success: false,
            ...
            if (request.side === 'sell') {
                const hasPosition = await this.validateSellOrderPosition(request.userId, request.tokenAddress, request.amount);
     ...
     */
    async validateBuyOrderBalance(userId, tokenAddress, amount, price, type) {
        try {
     ...
     */
    async validateSellOrderPosition(userId, tokenAddress, amount) {
        try {
