...
class HyperVMAMM {
    constructor(providerUrl, contractAddresses) {
        ...
        this.routerContract = new ethers_1.ethers.Contract(contractAddresses.router, ROUTER_ABI, this.provider);
    }
    /**
    ...
    connectSigner(signer) {
        this.signer = signer;
        this.routerContract = new ethers_1.ethers.Contract(this.contracts.router, ROUTER_ABI, signer);
    }
    /**
     ...
     */
    async calculateSwapOutput(pair, side, inputAmount) {
        try {
            const inputAmountWei = ethers_1.ethers.parseUnits(inputAmount.toString(), 18);
            let path;
            let amountsOut;
            if (side === 'buy') {
                // Buy INDEX with USDC
            ...
            const outputAmount = parseFloat(ethers_1.ethers.formatUnits(outputAmountWei, 18));
            // 현재 가격과 실효 가격 계산
    ...
    async executeSwap(params) {
        ...
        const { tokenIn, tokenOut, amountIn, slippageTolerance, recipient, deadline = Math.floor(Date.now() / 1000) + 60 * 20 // 20 minutes
         } = params;
        ...
        try {
            ...
            const reserves = await this.getPairReserves();
            const priceImpact = this.calculatePriceImpact(amountIn, expectedAmountOut.toString(), reserves, tokenIn);
            ...
            const tx = await this.routerContract.swapExactTokensForTokens(amountIn, minAmountOut, path, recipient, deadline);
     ...
     */
    async getAmountToReachPrice(pair, targetPrice, side) {
        try {
            const reserves = await this.getPairReserves();
            const currentPrice = await this.getSpotPrice(pair);
     ...
     */
    async executeSwapUntilPrice(pair, side, maxAmount, targetPrice) {
        try {
            // 실행 전 리저브 상태
            ...
            const swapParams = {
                ...
                amountIn: ethers_1.ethers.parseUnits(actualAmount.toString(), 18).toString(),
    ...
    async addLiquidity(params) {
        ...
        const { tokenA, tokenB, amountADesired, amountBDesired, slippageTolerance, recipient, deadline = Math.floor(Date.now() / 1000) + 60 * 20 } = params;
        ...
        try {
            ...
            // 3. Add liquidity
            const tx = await this.routerContract.addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, recipient, deadline);
    ...
    async getPairReserves() {
        const pairContract = new ethers_1.ethers.Contract(this.contracts.pair, PAIR_ABI, this.provider);
    ...
    async getSwapQuote(tokenIn, tokenOut, amountIn) {
        ...
        const amountsOut = await this.routerContract.getAmountsOut(amountIn, path);
        const reserves = await this.getPairReserves();
        const priceImpact = this.calculatePriceImpact(amountIn, amountsOut[1].toString(), reserves, tokenIn);
        return {
    ...
    async ensureTokenApproval(tokenAddress, amount) {
        ...
        if (allowance < BigInt(amount)) {
            ...
            const tx = await tokenContract.approve(this.contracts.router, ethers_1.ethers.MaxUint256);
            await tx.wait();
            console.log('✅ Token approved');
        }
    }
    calculatePriceImpact(amountIn, amountOut, reserves, tokenIn) {
