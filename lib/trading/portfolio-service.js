...
export class PortfolioService {
     ...
     */
    async getPortfolio(userId, userWalletAddress) {
        try {
    ...
    async getUserPositions(userId) {
        ...
        `)
            ...
            if (error) {
                console.error('❌ Failed to get positions from database:', error);
                return [];
            }
            ...
            const positions = await Promise.all(dbPositions.map(async (position) => {
                try {
                    ...
                    console.error(`❌ Failed to process position for ${position.token_address}:`, error);
                    return {
                        tokenAddress: position.token_address,
     ...
     * Get user balances
     */
    async getUserBalances(userId, walletAddress) {
        try {
     ...
     */
    async syncBalancesFromBlockchain(userId, walletAddress) {
        try {
            ...
            console.error('❌ Failed to sync balances from blockchain:', error);
        }
    }
    /**
     * Update user balances (called after trades)
     */
    async updateUserBalances(userId) {
        try {
     ...
     */
    calculateTotalValue(positions, balances) {
    ...
    async calculatePnL24h(userId) {
        try {
            ...
            console.error('❌ Failed to calculate 24h P&L:', error);
            return '0.00';
        }
    }
    /**
     * Update realized P&L
     */
    async updateRealizedPnL(userId, tokenAddress, realizedPnL) {
        try {
            await this.supabase
                .from('trading_positions')
                .update({
            ...
            console.error('❌ Failed to update realized P&L:', error);
        }
    }
    /**
