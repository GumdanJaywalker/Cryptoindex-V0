...
export class OnChainSettlementService {
    ...
    config;
    isProcessing = false;
    batchSize = 10; // Max trades per batch
    settlementQueue = [];
    constructor(config) {
        this.config = config;
        this.provider = new ethers.JsonRpcProvider(config.providerUrl);
        this.signer = new ethers.Wallet(config.privateKey, this.provider);
        this.settlementContract = new ethers.Contract(config.contractAddress, SETTLEMENT_ABI, this.signer);
    ...
    async queueTradeForSettlement(trade) {
        ...
        const pendingTrade = {
            ...
            sellAmount: ethers.parseUnits((parseFloat(trade.amount) * parseFloat(trade.price)).toString(), this.getTokenDecimals(sellToken)).toString(),
    ...
    async settleSingleTrade(trade) {
        try {
            ...
            // Execute settlement
            const tx = await this.settlementContract.settleTrade(trade.tradeId, trade.buyer, trade.seller, trade.buyToken, trade.sellToken, trade.buyAmount, trade.sellAmount, buyerNonce, sellerNonce);
            ...
            console.error(`❌ Failed to settle trade ${trade.redisTradeId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async settleBatchTrades(trades) {
        try {
            ...
            const buyerNonces = await Promise.all(buyers.map(buyer => this.settlementContract.getUserNonce(buyer)));
            const sellerNonces = await Promise.all(sellers.map(seller => this.settlementContract.getUserNonce(seller)));
            // Ensure all approvals
            for (const trade of trades) {
                await this.ensureApprovals(trade);
            }
            // Execute batch settlement
            const tx = await this.settlementContract.batchSettleTrades(tradeIds, buyers, sellers, tokensBuy, tokensSell, amountsBuy, amountsSell, buyerNonces, sellerNonces);
            ...
            console.error('❌ Failed to settle batch:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async updateTradeWithSettlement(redisTradeId, txHash) {
        ...
        if (error) {
            console.error('❌ Failed to update trade settlement info:', error);
        }
    }
    /**
...
export function getSettlementService(config) {
