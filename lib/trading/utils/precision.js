...
exports.TradingPairPrecision = exports.PrecisionMath = exports.TOKEN_PRECISION = void 0;
// 토큰별 정밀도 설정
exports.TOKEN_PRECISION = {
    'USDC': {
        symbol: 'USDC',
        decimals: 6,
...
class PrecisionMath {
     ...
     * 문자열을 정수로 변환 (정밀도 유지)
     */
    static toInteger(value, decimals) {
        if (!value || value === '0')
            return 0n;
     ...
     * 곱셈 (amount * price)
     */
    static multiply(amount, price, amountDecimals, priceDecimals, resultDecimals) {
     ...
     * 최소값 반환
     */
    static min(a, b, decimals) {
        return this.compare(a, b, decimals) <= 0 ? a : b;
    }
    /**
     * 최대값 반환
     */
    static max(a, b, decimals) {
        return this.compare(a, b, decimals) >= 0 ? a : b;
    }
    /**
     * 0인지 확인
     ...
     * 유효한 수량인지 검증
     */
    static isValidAmount(amount, tokenSymbol) {
        const precision = exports.TOKEN_PRECISION[tokenSymbol];
        if (!precision)
            return false;
     ...
     */
    static adjustToValidAmount(amount, tokenSymbol) {
        const precision = exports.TOKEN_PRECISION[tokenSymbol];
        if (!precision)
            return amount;
        const amountInt = this.toInteger(amount, precision.decimals);
 ...
 * 거래쌍별 정밀도 정보
 */
class TradingPairPrecision {
    static getPairInfo(pair) {
        const [baseSymbol, quoteSymbol] = pair.split('-');
        const base = exports.TOKEN_PRECISION[baseSymbol];
        const quote = exports.TOKEN_PRECISION[quoteSymbol];
        if (!base || !quote) {
     ...
     * 체결 금액 계산 (정밀도 유지)
     */
    static calculateTradeValue(amount, price, pair) {
        const { base, quote } = this.getPairInfo(pair);
        return PrecisionMath.multiply(amount, price, base.decimals, quote.decimals, quote.decimals // 결과는 quote 토큰 정밀도
        );
    }
    /**
     * 부분체결 시 나머지 수량 조정
     */
    static adjustRemainingAmount(originalAmount, filledAmount, pair) {
        const { base } = this.getPairInfo(pair);
        const remaining = PrecisionMath.subtract(originalAmount, filledAmount, base.decimals);
        const adjustedRemaining = PrecisionMath.adjustToValidAmount(remaining, base.symbol);
        return {
...
exports.TradingPairPrecision = TradingPairPrecision;
