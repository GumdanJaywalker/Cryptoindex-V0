...
export class PrecisionUtils {
    static PRECISION_DECIMALS = 18;
    static BASIS_POINTS = 10000;
    /**
     * Calculate precise remaining amount after partial fill
     */
    static calculateRemainingAmount(originalAmount, filledAmount, decimals = 18) {
        try {
     ...
     */
    static calculateAverageFillPrice(fills, decimals = 18) {
        try {
            if (fills.length === 0)
                return '0';
            let totalValue = 0n;
            ...
            console.error('❌ Error calculating average fill price:', error);
     ...
     */
    static isAmountEqual(amount1, amount2, decimals = 18, toleranceBps = 1 // 0.01% tolerance
    ) {
        try {
     ...
     */
    static validateAndFormatAmount(amount, decimals = 18, minAmount = '0.000001') {
        try {
     ...
     */
    static calculateFillPercentage(filledAmount, totalAmount, decimals = 18) {
        try {
            const filled = ethers.parseUnits(filledAmount, decimals);
     ...
     */
    static determineOrderStatus(filledAmount, totalAmount, decimals = 18) {
        try {
            const filled = ethers.parseUnits(filledAmount, decimals);
            const total = ethers.parseUnits(totalAmount, decimals);
            if (filled === 0n)
                return 'pending';
            if (this.isAmountEqual(filledAmount, totalAmount, decimals))
                return 'filled';
            if (filled > 0n && filled < total)
                return 'partial';
            return 'pending';
        }
        catch (error) {
     ...
     */
    static safeDivide(numerator, denominator, decimals = 18) {
        try {
            ...
            console.error('❌ Error in safe division:', error);
     ...
     */
    static safeMultiply(value1, value2, decimals = 18) {
        try {
            ...
            console.error('❌ Error in safe multiplication:', error);
     ...
     */
    static safeAdd(value1, value2, decimals = 18) {
        try {
            const val1 = ethers.parseUnits(value1, decimals);
            const val2 = ethers.parseUnits(value2, decimals);
            ...
            console.error('❌ Error in safe addition:', error);
     ...
     */
    static safeSubtract(value1, value2, decimals = 18) {
        try {
            const val1 = ethers.parseUnits(value1, decimals);
            const val2 = ethers.parseUnits(value2, decimals);
            if (val2 > val1) {
            ...
            console.error('❌ Error in safe subtraction:', error);
