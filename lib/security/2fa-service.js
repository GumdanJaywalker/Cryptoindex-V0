...
export class TwoFactorService {
    ...
    SETUP_TOKEN_EXPIRY = 300000; // 5 minutes
    constructor() {
     ...
     */
    async generateTwoFactorSetup(userId) {
        try {
     ...
     */
    async verifyTwoFactorSetup(userId, setupToken, totpCode) {
        try {
            // Get setup data
     ...
     */
    async verifyTwoFactorCode(userId, code, allowBackupCode = true) {
        try {
    ...
    async generateNewBackupCodes(userId) {
        try {
            ...
            const { error } = await this.supabase
                ...
                .eq('user_id', userId);
            if (error) {
                throw new Error(`Failed to generate new backup codes: ${error.message}`);
            }
     ...
     */
    async disableTwoFactor(userId, totpCode) {
        try {
    ...
    async cleanupExpiredSetups() {
        try {
            ...
            if (error) {
                console.error('❌ Failed to cleanup expired 2FA setups:', error);
            }
            else {
            ...
            console.error('❌ Error during 2FA setup cleanup:', error);
        }
    }
    /**
     * Get 2FA statistics
...
export const generateTwoFactorSetup = async (userId) => {
    const service = TwoFactorService.getInstance();
    return service.generateTwoFactorSetup(userId);
};
export const verifyTwoFactorSetup = async (userId, setupToken, totpCode) => {
    const service = TwoFactorService.getInstance();
    return service.verifyTwoFactorSetup(userId, setupToken, totpCode);
};
export const verifyTwoFactorCode = async (userId, code, allowBackupCode) => {
    const service = TwoFactorService.getInstance();
