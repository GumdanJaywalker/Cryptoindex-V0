...
export class AuditLogger {
     ...
     */
    async generateSecurityReport(startDate, endDate, userId) {
        try {
            ...
            const metrics = {
                ...
                failedAuthentications: logs.filter(log => log.event_category === 'authentication' && log.outcome === 'failure').length,
                suspiciousTransactions: logs.filter(log => log.event_category === 'transaction' && log.event_data?.riskScore > 70).length,
                dataAccessEvents: logs.filter(log => log.action?.includes('read') || log.action?.includes('access')).length,
    ...
    calculateExpiryDate(retentionCategory) {
        const retentionDays = this.RETENTION_PERIODS[retentionCategory];
    ...
    async logToCriticalSystem(logEntry) {
        try {
            ...
            console.error('❌ Failed to log to critical system:', error);
        }
    }
    ...
    checkAuditTrailCompleteness(logs, startDate, endDate) {
    ...
    generateRecommendations(metrics, security, logs) {
