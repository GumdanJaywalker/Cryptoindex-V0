...
export class FraudDetectionService {
    ...
    async analyzeTransaction(userId, transactionData) {
        try {
            ...
            if (destinationRisk.severity !== 'low')
                riskFactors.push(destinationRisk);
     ...
     */
    async updateUserBehaviorPattern(userId, transactionData) {
        try {
            // Get current pattern
            ...
            console.error('❌ Failed to update user behavior pattern:', error);
        }
    }
    /**
     * Get user behavior pattern
     ...
     */
    async buildInitialBehaviorPattern(userId) {
        try {
            ...
            console.error('❌ Failed to build initial behavior pattern:', error);
            return this.getDefaultBehaviorPattern(userId);
        }
    }
    /**
     ...
     */
    analyzeAmountDeviation(amount, pattern) {
     ...
     */
    async analyzeGeolocation(ipAddress, pattern) {
        try {
     ...
     */
    analyzeDeviceFingerprint(deviceFingerprint, pattern) {
     ...
     */
    async analyzeTransactionVelocity(userId) {
        try {
     ...
     */
    analyzeDestinationAddress(destinationAddress, pattern) {
     ...
     * Get ML prediction if available
     */
    async getMlPrediction(userId, transactionData, userPattern) {
     ...
     */
    extractFeatures(userId, transactionData, userPattern) {
        return {
     ...
     * Combine rule-based and ML scores
     */
    combineScores(ruleBasedScore, mlScore) {
    ...
    getDefaultBehaviorPattern(userId) {
        return {
            userId,
    ...
    getMostFrequentValues(values, limit) {
    ...
    findNearestLocation(currentLocation, locationHistory) {
    ...
    async getTransactionCount(userId, since) {
    ...
    calculateDaysSinceLastTransaction(userPattern) {
        const daysSince = (Date.now() - userPattern.lastUpdated.getTime()) / (1000 * 60 * 60 * 24);
        return Math.floor(daysSince);
    }
    async saveUserBehaviorPattern(userId, pattern) {
    ...
    calculateUpdatedPattern(current, newData) {
    ...
    updateTypicalHours(current, newHour) {
    ...
    updateFrequentDestinations(current, newDestination) {
        const updated = [...current];
    ...
    updatePreferredNetworks(current, newNetwork) {
        const updated = [...current];
    ...
    updateDeviceFingerprints(current, newFingerprint) {
        if (!newFingerprint)
            return current;
        const updated = [...current];
...
export const analyzeFraudRisk = async (userId, transactionData) => {
    const service = FraudDetectionService.getInstance();
    return service.analyzeTransaction(userId, transactionData);
};
export const updateBehaviorPattern = async (userId, transactionData) => {
    const service = FraudDetectionService.getInstance();
