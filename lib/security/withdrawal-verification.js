...
export class WithdrawalVerificationService {
    ...
    supabase;
    twoFactorService;
    VERIFICATION_EXPIRY = 300000; // 5 minutes
     ...
     */
    async verifyWithdrawalSignature(signature, walletAddress, destinationAddress, amount, timestamp) {
        try {
     ...
     */
    async createVerificationRequest(userId, walletAddress, amount, destinationAddress, method) {
        try {
     ...
     */
    async verifyWithdrawalCode(verificationId, code) {
        try {
            ...
            switch (verification.verification_method) {
                case '2fa':
                    ...
                    const twoFaResult = await this.twoFactorService.verifyTwoFactorCode(verification.user_id, code, true // Allow backup codes
                    );
     ...
     */
    async sendVerificationCode(userId, code, method) {
        try {
     ...
     */
    async requiresAdditionalVerification(userId, amount, destinationAddress) {
        const reasons = [];
        const amountNum = parseFloat(amount);
        // Check if user has 2FA enabled
        const has2FA = await this.twoFactorService.isTwoFactorEnabled(userId);
        let preferredMethod = has2FA ? '2fa' : 'email';
        ...
        const { data: previousWithdrawals } = await this.supabase
            ...
            .eq('status', 'completed');
        const usedAddresses = (previousWithdrawals || []).map(tx => tx.metadata?.destination_address).filter(Boolean);
    ...
    async getVerificationStats() {
        try {
            ...
            const failedVerifications = verifications.filter(v => !v.is_verified && v.attempt_count >= this.MAX_VERIFICATION_ATTEMPTS).length;
...
export const verifyWithdrawalSignature = async (signature, walletAddress, destinationAddress, amount, timestamp) => {
...
export const createWithdrawalVerification = async (userId, walletAddress, amount, destinationAddress, method) => {
    const service = WithdrawalVerificationService.getInstance();
    return service.createVerificationRequest(userId, walletAddress, amount, destinationAddress, method);
};
export const verifyWithdrawalCode = async (verificationId, code) => {
    const service = WithdrawalVerificationService.getInstance();
