...
import { verifyPrivyJWT, extractPrivyUserId } from '@/lib/auth/privy-jwt';
/**
 * 인증이 필요한 라우트 목록
 */
 ...
 */
export async function extractPrivyAuthFromRequest(request) {
    try {
        ...
        return {
            ...
            user: {
                ...
                authType: payload.wallet ? 'wallet' : 'email',
 ...
 */
export async function requirePrivyAuth(request) {
    const authResult = await extractPrivyAuthFromRequest(request);
    if (!authResult.authenticated) {
        return NextResponse.json({ error: authResult.error, authenticated: false }, { status: 401 });
    }
    return {
 ...
 * 관리자 권한 확인 (Privy 기반)
 */
export async function requirePrivyAdminAuth(request) {
    const authResult = await extractPrivyAuthFromRequest(request);
    if (!authResult.authenticated) {
        return NextResponse.json({ error: authResult.error, authenticated: false }, { status: 401 });
    }
    ...
    if (!isAdmin) {
        return NextResponse.json({ error: 'Admin access required', authenticated: true, authorized: false }, { status: 403 });
    }
    return {
        user: authResult.user,
        privyToken: authResult.privyToken
...
export function rateLimit(identifier, maxRequests = 999999999, // 사실상 무제한
 ...
 */
export function rateLimitByIP(request, maxRequests = 999999999, windowMs = 15 * 60 * 1000) {
    if (process.env.NODE_ENV === 'development' || process.env.DISABLE_RATE_LIMIT === 'true') {
        return { allowed: true, remaining: 999999999, resetTime: Date.now() + windowMs };
    }
 ...
 */
export function rateLimitByUser(userId, maxRequests = 999999999, windowMs = 60 * 60 * 1000) {
    if (process.env.NODE_ENV === 'development' || process.env.DISABLE_RATE_LIMIT === 'true') {
        return { allowed: true, remaining: 999999999, resetTime: Date.now() + windowMs };
    }
