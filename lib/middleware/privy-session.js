 ...
 */
export async function getPrivySessionInfo(request) {
    try {
 ...
 */
export async function validatePrivySession(request) {
    const sessionInfo = await getPrivySessionInfo(request);
    if (!sessionInfo.isValid) {
        return {
            isValid: false,
            response: NextResponse.json({ success: false, error: sessionInfo.error || 'Invalid session' }, { status: 401 })
        };
    }
 ...
 */
export async function handlePrivyLogout(request) {
    try {
        const sessionInfo = await getPrivySessionInfo(request);
 ...
 */
export async function checkSessionSecurity(request) {
    try {
        const sessionInfo = await getPrivySessionInfo(request);
