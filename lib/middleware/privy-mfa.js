...
export async function verifyPrivyMfaAuth(request, config = {
    requireMfa: true,
    ...
    try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            return {
                ...
                response: NextResponse.json({ success: false, error: 'No authorization token provided' }, { status: 401 })
            };
        }
        ...
        console.error('❌ Privy MFA verification failed:', error);
        await auditLogger.logAuthentication({
        ...
        return {
            ...
            isMfaRequired: false,
            isMfaCompleted: false,
            response: NextResponse.json({ success: false, error: 'Authentication verification failed' }, { status: 500 })
        };
    }
}
/**
 ...
 */
function extractMfaStatus(claims) {
 ...
 */
function shouldRequireMfa(claims, config) {
 ...
 */
export function createMfaMiddleware(config) {
 ...
 */
export async function requireFreshMfa(request, maxAgeMinutes = 5) {
 ...
 */
export async function checkPrivyMfaStatus(userId) {
    try {
        ...
        console.error('❌ Failed to check Privy MFA status:', error);
        return {
            enabled: false,
            methods: [],
            canEnable: false
