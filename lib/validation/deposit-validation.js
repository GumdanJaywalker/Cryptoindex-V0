...
export class DepositValidator {
    ...
    validateAmount(amount) {
        ...
        if (amountNum > 10000) {
            warnings.push('Large deposit detected. Please ensure you have sufficient funds and are aware of the risks.');
        }
        // Add warning for amounts close to minimum
        if (amountNum >= this.config.minAmount && amountNum < this.config.minAmount + 1) {
            warnings.push('Amount is close to minimum requirement. Consider depositing slightly more to account for potential fees.');
        }
        return {
     ...
     */
    validateBalance(depositAmount, userBalance) {
        const errors = [];
        const warnings = [];
        ...
        if (amountNum > balanceNum * 0.9) {
            warnings.push('You are depositing most of your balance. Consider keeping some tokens for gas fees.');
        }
        return {
            isValid: errors.length === 0,
            errors,
...
export const validateDepositAmount = (amount) => depositValidator.validateAmount(amount);
export const validateWalletAddress = (address) => depositValidator.validateWalletAddress(address);
export const validateDepositRequest = (request) => depositValidator.validateDepositRequest(request);
// Constants
