...
export class WithdrawalLimitsService {
     ...
     */
    async validateWithdrawal(userId, amount, destinationAddress, walletAddress) {
        const errors = [];
        const warnings = [];
        try {
    ...
    async getLifetimeUsage(userId) {
        const { data: withdrawals, error } = await this.supabase
            ...
            .eq('status', 'completed');
        if (error) {
            console.error('❌ Failed to get lifetime usage:', error);
            return 0;
        }
        return withdrawals?.reduce((sum, tx) => sum + parseFloat(tx.amount), 0) || 0;
    }
     ...
     */
    async checkSuspiciousActivity(userId, amount, destinationAddress) {
        const warnings = [];
        try {
     ...
     * Check balance availability
     */
    async checkBalanceAvailability(walletAddress, amount) {
        try {
            ...
            console.error('❌ Failed to check balance availability:', error);
            return {
                sufficient: false,
                available: '0'
            };
     ...
     * Check for pending withdrawals
     */
    async checkPendingWithdrawals(userId) {
        try {
     ...
     * Check tier-specific requirements
     */
    async checkTierRequirements(userId, tier) {
        const violations = [];
        try {
            const { data: user, error } = await this.supabase
                .from('users')
     ...
     */
    async getAverageWithdrawalAmount(userId) {
        try {
            const { data: withdrawals, error } = await this.supabase
                .from('transactions')
                .select('amount')
            ...
            console.error('❌ Failed to get average withdrawal amount:', error);
            return 100;
        }
    }
    /**
     ...
     */
    async getWithdrawalLimitsSummary(userId) {
        try {
            const userTier = await this.getUserTier(userId);
            const limits = await this.getUserLimits(userId);
            const nextTier = this.getNextTier(userTier.tier);
            ...
            let upgradeBenefits = [];
            if (nextTier) {
                const nextTierLimits = this.DEFAULT_LIMITS[nextTier];
            ...
            console.error('❌ Failed to get withdrawal limits summary:', error);
            throw error;
        }
    }
}
// Export utility functions
export const validateWithdrawal = async (userId, amount, destinationAddress, walletAddress) => {
...
export const getUserWithdrawalLimits = async (userId) => {
    const service = WithdrawalLimitsService.getInstance();
...
export const getWithdrawalLimitsSummary = async (userId) => {
    const service = WithdrawalLimitsService.getInstance();
