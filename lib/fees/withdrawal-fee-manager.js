...
export class WithdrawalFeeManager {
     ...
     */
    async calculateWithdrawalFee(userId, grossAmount, walletAddress) {
        try {
            // Base calculation
    ...
    async getUserTradingVolume(userId) {
        try {
            ...
            if (error) {
                console.error('❌ Failed to get user trading volume:', error);
                return 0;
            }
            return transactions.reduce((sum, tx) => sum + parseFloat(tx.amount), 0);
        }
        catch (error) {
            console.error('❌ Error getting user trading volume:', error);
            return 0;
        }
    }
    /**
     ...
     */
    getApplicableFeeDiscount(userVolume) {
     ...
     */
    async recordWithdrawalFee(userId, transactionId, feeCalculation) {
        try {
            ...
            if (error) {
                console.error('❌ Failed to record withdrawal fee:', error);
            }
        }
        catch (error) {
    ...
    async getUserFeeDiscount(userId) {
        try {
            ...
            console.error('❌ Error getting user fee discount:', error);
            return {
                currentVolume: 0
            };
        }
    ...
    async getWithdrawalFeeStats() {
        try {
            ...
            console.error('❌ Error getting withdrawal fee stats:', error);
            return {
                totalFeesCollected: 0,
                totalFeesWaived: 0,
                averageFeePerWithdrawal: 0,
     ...
     */
    updateFeeConfiguration(newConfig) {
        this.config = {
            ...this.config,
            ...newConfig
        };
...
export const calculateWithdrawalFee = async (userId, grossAmount, walletAddress) => {
...
export const getUserFeeDiscount = async (userId) => {
    const feeManager = WithdrawalFeeManager.getInstance();
