 ...
 */
export function useTransactionUpdates(options = {}) {
    ...
    const { transactionId, walletAddress, pollInterval = 10000, // 10 seconds
    enableRealtime = true } = options;
    /**
     ...
     */
    const subscribe = useCallback((txId) => {
        if (!enableRealtime || subscriptions.has(txId))
            return;
        ...
        const channel = supabase
            ...
            setTransactions(prev => {
                const updated = prev.map(tx => tx.id === txId ? { ...tx, ...payload.new } : tx);
                return updated;
            });
            setLastUpdate(new Date());
        })
     ...
     */
    const unsubscribe = useCallback((txId) => {
     ...
     */
    const refreshTransaction = useCallback(async (txId) => {
        if (!authenticated || !user)
            return;
        try {
            ...
            setTransactions(prev => {
                const updated = prev.map(tx => tx.id === txId ? data.transaction : tx);
                return updated;
            });
            setLastUpdate(new Date());
        }
    ...
    useEffect(() => {
        if (!enableRealtime || !authenticated || transactions.length === 0)
            return;
    ...
    useEffect(() => {
        if (!enableRealtime || !authenticated)
            return;
        const activeTransactions = transactions.filter(tx => tx.status === 'pending' || tx.status === 'processing');
        ...
        const completedTransactions = transactions.filter(tx => tx.status === 'completed' || tx.status === 'failed' || tx.status === 'cancelled');
...
export class TransactionNotificationService {
     ...
     */
    async showTransactionNotification(transaction, previousStatus) {
     ...
     */
    closeNotification(transactionId) {
