...
export class PostgresOrderbookSync {
    ...
    async syncOrder(order) {
        try {
            ...
            console.error('Failed to sync order to PostgreSQL:', error);
    ...
    async syncTrade(trade) {
        try {
            ...
            console.error('Failed to sync trade to PostgreSQL:', error);
        }
    }
    /**
     * 배치로 여러 주문 동기화
     */
    async syncOrdersBatch(orders) {
        if (orders.length === 0)
            return;
        try {
            ...
            const { error } = await supabase
                ...
                .upsert(orderData);
            if (error) {
                console.error('Batch order sync error:', error);
                throw error;
            }
        }
        catch (error) {
    ...
    async syncTradesBatch(trades) {
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .insert(tradeData);
            if (error) {
                console.error('Batch trade sync error:', error);
                throw error;
            }
        }
        catch (error) {
            console.error('Failed to batch sync trades:', error);
        }
    }
    /**
     * 사용자 주문 통계 업데이트
    ...
    async updateUserStats(userId) {
        try {
            ...
            if (stats) {
                const totalVolume = stats.reduce((sum, trade) => sum + (parseFloat(trade.amount) * parseFloat(trade.price)), 0);
    ...
    async updateDailyStats(pair) {
        try {
            ...
            if (todayTrades) {
                const volume = todayTrades.reduce((sum, trade) => sum + (parseFloat(trade.amount) * parseFloat(trade.price)), 0);
