...
export class WithdrawalMonitoringService {
    ...
    async monitorPendingWithdrawals() {
        ...
        try {
            ...
            if (error) {
                result.errors.push(`Failed to fetch pending withdrawals: ${error.message}`);
                return result;
            }
     ...
     * Check withdrawal status on Hyperliquid
     */
    async checkHyperliquidWithdrawalStatus(withdrawal) {
        try {
            ...
            console.error('❌ Failed to check Hyperliquid status:', error);
            return {
                completed: false,
                failed: true,
     ...
     * Complete withdrawal
     */
    async completeWithdrawal(withdrawal, statusCheck) {
        try {
            ...
            console.error(`❌ Failed to complete withdrawal ${withdrawal.id}:`, error);
            throw error;
        }
    }
    /**
     * Fail withdrawal
     */
    async failWithdrawal(withdrawal, error) {
        try {
            ...
            console.error(`❌ Failed to fail withdrawal ${withdrawal.id}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async handleWithdrawalTimeout(withdrawal) {
        try {
            ...
            console.error(`❌ Failed to handle timeout for withdrawal ${withdrawal.id}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async updateLastCheckTime(transactionId) {
        try {
     ...
     */
    async logWithdrawalEvent(userId, eventType, metadata) {
        try {
            await this.supabase
            ...
            console.error('❌ Failed to log withdrawal event:', error);
        }
    }
    /**
     ...
     */
    async forceCheckWithdrawal(transactionId) {
        try {
            ...
            return {
                success: false,
                status: 'error',
                error: error instanceof Error ? error.message : 'Force check failed'
            };
        }
    }
}
...
export const forceCheckWithdrawal = async (transactionId) => {
    const service = WithdrawalMonitoringService.getInstance();
