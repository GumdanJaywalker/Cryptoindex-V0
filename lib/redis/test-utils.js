...
export async function testRedisBasicOperations() {
    ...
    try {
        ...
        await redis.zadd('test:orderbook:bids', 150000, 'order_123', // 가격 1.50000
...
export async function testOrderbookStructure() {
    ...
    try {
        ...
        // 매수 주문 추가 (높은 가격 우선)
        await redis.zadd(`orderbook:${pair}:bids`, 150000, 'buy_order_1', // $1.50000
        149500, 'buy_order_2', // $1.49500
        148000, 'buy_order_3' // $1.48000
        );
        // 매도 주문 추가 (낮은 가격 우선)
        await redis.zadd(`orderbook:${pair}:asks`, 151000, 'sell_order_1', // $1.51000
        ...
        console.error('❌ Orderbook test failed:', error);
        return false;
    }
}
// Pub/Sub 테스트
