...
// 거래쌍별 정밀도 설정
export const TRADING_PAIR_CONFIGS = {
    'HYPERINDEX-USDC': {
...
export class AdvancedPrecisionMath {
     ...
     * 내부 계산용 고정밀도 (24자리)
     */
    static INTERNAL_DECIMALS = 24;
    /**
     * 가격을 내부 정밀도로 변환
     */
    static priceToInternal(price, priceDecimals) {
        return this.toFixed(price, priceDecimals, this.INTERNAL_DECIMALS);
    }
    /**
     * 수량을 내부 정밀도로 변환
     */
    static amountToInternal(amount, tokenDecimals) {
        return this.toFixed(amount, tokenDecimals, this.INTERNAL_DECIMALS);
    }
    /**
     * 정밀도 변환
     */
    static toFixed(value, fromDecimals, toDecimals) {
        if (!value || value === '0')
            return 0n;
     ...
     * 내부 정밀도에서 문자열로 변환
     */
    static fromInternal(value, targetDecimals) {
        if (value === 0n)
            return '0';
        // 내부 정밀도에서 대상 정밀도로 변환
     ...
     */
    static calculateTradeValue(price, // 9자리 가격
    amount, // 18자리 수량
    config) {
        // 내부 정밀도로 변환
     ...
     */
    static calculateOptimalAmount(price, // 9자리 가격
    desiredValue, // 원하는 USDC 금액 (6자리)
    config) {
        // 내부 정밀도로 변환
        const priceInternal = this.priceToInternal(price, config.priceDecimals);
     ...
     * 주문 유효성 검증
     */
    static validateOrder(price, amount, config) {
        try {
            // 가격 유효성 체크
     ...
     * 가격이 유효한 틱 단위인지 확인
     */
    static isValidPrice(price, config) {
        try {
     ...
     * 부분 체결 시 나머지 계산
     */
    static calculatePartialFill(originalAmount, filledAmount, price, config) {
        // 내부 정밀도로 계산
     ...
     * 가격 비교 (9자리 정밀도)
     */
    static comparePrice(priceA, priceB, config) {
     ...
     * 수량 비교 (토큰 정밀도)
     */
    static compareAmount(amountA, amountB, tokenDecimals) {
...
export class TradingPairHelper {
     ...
     * 스마트 주문 금액 조정
     */
    static smartOrderAdjustment(price, desiredAmount, pair) {
     ...
     * 호가 단위 스냅
     */
    static snapToTickSize(price, pair) {
        const config = this.getConfig(pair);
...
export class PrecisionFormatter {
    ...
    static formatAmount(amount, tokenDecimals) {
