...
export class TestnetFaucetService {
    ...
    getEVMProvider() {
        ...
        console.log('🔍 Debugging wallet detection:', {
            ...
            allWindowKeys: Object.keys(window).filter(key => key.toLowerCase().includes('ethereum') || key.toLowerCase().includes('metamask'))
        });
        // Check if there are multiple providers
        if (windowEthereum?.providers && Array.isArray(windowEthereum.providers)) {
            console.log('🔍 Multiple providers detected:', windowEthereum.providers.map((p, index) => ({
                index,
            ...
            const metaMaskProvider = windowEthereum.providers.find((p) => {
                return (p.isMetaMask &&
                    !p.isPhantom &&
                    !p.isSolana &&
            ...
            const evmProvider = windowEthereum.providers.find((p) => {
        ...
        if (window.web3?.currentProvider?.isMetaMask && !window.web3?.currentProvider?.isPhantom) {
    ...
    async addTestnetNetworks() {
        ...
        try {
            ...
            if (error.code === 4001) {
                ...
                console.error('❌ Failed to add Arbitrum Sepolia:', error);
            }
        }
        try {
            ...
            if (error.code === 4001) {
                ...
                console.error('❌ Failed to add Hyperliquid Testnet:', error);
            }
        }
        return results;
    }
     ...
     */
    async checkTestnetBalances(walletAddress) {
        try {
            ...
            const usdcContract = new ethers.Contract(ARBITRUM_SEPOLIA_CONFIG.tokens.usdc.address, ['function balanceOf(address) view returns (uint256)'], arbProvider);
            ...
            console.error('❌ Failed to check testnet balances:', error);
            return {
